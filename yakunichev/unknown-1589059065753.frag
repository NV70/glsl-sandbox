...
void main(){
    ...
    for(float i=0.; i<10.; i++) {
        float c = circle(
            st,
            vec2(
                exponentialOut((i/10. * (cos(u_time * 3.) + 0.140) * rand(st)) + 0.106),
                exponentialInOut((i/10. * (sin(u_time * 2.))) * rand(st) + 0.512)
            ),
            exponentialInOut(abs(sin(u_time))) / 4.,
            0.266
        );
        color = mix(
            color,
            vec3(
                exponentialInOut(i/10. * cos(u_time) + rand(st)),
                cubicInOut(cos(u_time) * rand(st)),
                i/10. * abs(sin(u_time))
            ),
            c
        );
