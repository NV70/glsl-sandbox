...
void main(){
    ...
    color = mix(color, vec3(0.,0.760,0.),
                circle(space3, vec2(0.5), 0.475, 0.02, 0.5) * smoothstep(0.475, 0.48, distance(space3, vec2(0.5))) * hline(space3, 0.5, 0.85) * 0.5
               );
    
        color = mix(color, vec3(0.441,0.691,0.725),
                circle(space3, vec2(0.5), 0.475, 0.02, 0.5) * smoothstep(0.475, 0.48, distance(space3, vec2(0.5))) * hline(space3, 0.5, 0.15) * 0.5
               );
    ...
    space4 += vec2(0.5);
    color = mix(color, vec3(0.441,0.691,0.725),
                circle(space4, vec2(0.5), 0.375, 0.02, 0.5) * smoothstep(0.375, 0.38, distance(space4, vec2(0.5))) * hline(space4, 0.5, 0.85) * 0.5
               );
    
        color = mix(color, vec3(0.,0.760,0.),
                circle(space4, vec2(0.5), 0.375, 0.02, 0.5) * smoothstep(0.375, 0.38, distance(space4, vec2(0.5))) * hline(space4, 0.5, 0.15) * 0.5
               );
    ...
    for (int i = CIRCLES; i > 0; i--) {
        ...
        float y = scale((cos(u_time + width + 0.3575)), width, 1. - width, -1., 1.);
        color = mix(
        	color,
