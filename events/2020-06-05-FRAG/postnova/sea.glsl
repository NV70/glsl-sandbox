...
float scene(vec3 position){    
    ...
    float ground = st.y + sin(position.x*2. * sin(iTime/5.)) / 5. 
                              + cos(position.z*10. * fract (iTime/5.)) / 12. + 2.0 ;
...
vec4 trace (vec3 origin, vec3 direction){
    ...
    return vec4(0.890+sin(iTime),0.583+sin(iTime),0.540,1.000);// Background color.
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 camOrigin = vec3(0., 0., -1.)*random(iTime/4.);
