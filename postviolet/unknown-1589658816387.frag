...
float rect(vec2 st, vec2 size, float border, float posX, float posY) {
    ...
    size = ((1. - size) / 2.);
    st.x = (st.x-posX+0.5)-(size.x/50.);
    st.y = st.y-posY+0.5-(size.y/50.);
...
void main(){
  ...
  vec2 space3 = st;
  space3 -= vec2(0.5);
  space3 = rotate2d( 0.5 ) * space3;
  space3 += vec2(0.5);  
  float r1 = rect(space1, vec2(0.400,0.400), 0.002, 0.5, 0.5);
  float x1 = cross(space3, 0.500);

  color = mix(color, vec3 (1.0, 0.0, 0.0), r1); 
  color = mix(color, vec3(0.0,0.0,0.0), x1);

  float c2 = circle(st, 0.436, vec2(0.5,0.5), 2.492);
  color = mix(color, vec3(0.000,0.0,0.0),c2); 
