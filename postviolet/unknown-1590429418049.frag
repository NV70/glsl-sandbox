...
float cross(in vec2 _st, float _size){
    return  box(_st, vec2(_size+(noise(u_time*3.)),_size/4.)) +
            box(_st, vec2(_size/4.,_size+(noise(u_time*3.))));
}
...
void main(){
  ...
  float c = circle(space1,0.604, vec2(0.5,0.5), 1.5*(noise(u_time*3.)));
  color = mix(color, vec3(0.620,0.479,0.601),c);
  
    vec2 space3 = st;
  space3 -= vec2(0.5);
  space3 = rotate2d((noise(u_time*5.99999)))*space3;
  space3 += vec2(0.5);
  ...
  vec2 space2 = st;
  space2 -= vec2(0.5);
  space2 = rotate2d((noise(u_time*6.)))*space2;
  space2 += vec2(0.5);
